...
<div class="navbar">
    ...
    <a class="active" href="https://s4515529-infs7901.uqcloud.net/000_q16.php">Current Query</a>
</div>

<div class="row">
  <div class="main"
    <p>Information about Slots Advertiser perform on in the past week</p>
   
    <br>
    <p><b>SQL Query</b></p>
    <p><i>CREATE VIEW advertiser_performance_slot(Performing_Slot) AS</p>
    <p>SELECT COUNT(DISTINCT ad.SlotID ) AS Performing_Slot</p>
    <p>FROM clicks LEFT OUTER JOIN ad</p>
    <p>ON clicks.AdID = ad.AdID</p>
    <p>LEFT OUTER JOIN webpage</p>
    <p>ON ad.SlotID = webpage.SlotID</p>
    <p>WHERE Date >= '2018-05-07' AND Date <= '2018-05-13'</p>
    <p>GROUP BY clicks.AdvertiserID)</p>
    <br>
    <p>SELECT $aggregate(Performing_Slot)</p>
    <p>FROM advertiser_performance_slot</i></p>
    <br>
    <?php 
    ...
    <table class="table">
      <thead>
            <tr>
                <th scope="col">Slots Performed on</th>
            </tr>
        </thead>
      <tbody id="Table">
        <?php
        ...
        // get data
        $query = "SELECT $aggregate(Performing_Slot) 
            FROM advertiser_performance_slot";
        $result = mysqli_query($conn, $query);
        // put all our results into a html table
        while ($rows = mysqli_fetch_array($result)) {
            echo "<tr>";
            echo "<td>".$rows["$aggregate(Performing_Slot)"]."</td>";
            echo "</tr>";
          }
        }
            ?>
    ...
    <form action="" method="post">
        <br>
        <p>Enter MAX, MIN or AVG </p>
        <input type = "text" name = "aggregate" placeholder="Aggregation method"><br><br>
