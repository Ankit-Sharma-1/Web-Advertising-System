...
<div class="navbar">
    ...
    <a class="active" href="https://s4515529-infs7901.uqcloud.net/000_q5.php">Current Query</a>
</div>

<div class="row">
  <div class="main"
    <p>Generate Advertiser Bill for selected date range.</p>
    
    <br>
    <p><b>SQL Query</b></p>
    <p><i>SELECT clicks.AdvertiserID, ROUND(SUM(SlotClickCost),2)</p>
    <p>FROM clicks LEFT OUTER JOIN ad</p>
    <p>ON clicks.AdID = ad.AdID </p>
    <p>LEFT OUTER JOIN webpage </p>
    <p>ON Ad.SlotID = webpage.SlotID </p>
    <p>WHERE Date >= '$start_date' AND Date <= '$end_date'"</p>
    <p>GROUP BY clicks.AdvertiserID</p>
    <p>ORDER BY Total_Cost DESC</p>
    <p>LIMIT 5</i></p>
    <br>
    <?php 
    ...
    <table class="table">
      ...
      <tbody id="Table">
        ...
        // get data
        $query = "SELECT clicks.AdvertiserID, ROUND(SUM(SlotClickCost),2)
            ...
            ORDER BY ROUND(SUM(SlotClickCost),2) DESC
            LIMIT 5";
