...
<div class="navbar">
    ...
    <a class="active" href="https://s4515529-infs7901.uqcloud.net/000_q11.php">Current Query</a>
</div>

<div class="row">
  <div class="main"
    <p>Information about clicks at Advertiser grain.</p>
   
    <br>
    <p><b>SQL Query</b></p>
    <p><i>CREATE VIEW advertiser_count_clicks(Click_Count) AS(</p>
    <p>SELECT COUNT(ClickID) AS Click_Count</p>
    <p>FROM clicks</p>
    <p>WHERE Date >=  '2018-05-07' AND Date <= '2018-05-13'</p>
    <p>GROUP BY AdvertiserID)</p>
    <br>
    <p>SELECT $aggregate(Click_Count)</p>
    <p>FROM advertiser_count_clicks</i></p>
    <br>
    <?php 
    ...
    <table class="table">
      <thead>
            <tr>
                <th scope="col">Aggregated Clicks</th>
            </tr>
        </thead>
      <tbody id="Table">
        <?php
        ...
        // get data
        $query = "SELECT $aggregate(Click_Count) 
            FROM advertiser_count_clicks";
        $result = mysqli_query($conn, $query);
        // put all our results into a html table
        while ($rows = mysqli_fetch_array($result)) {
            echo "<tr>";
            echo "<td>".$rows["$aggregate(Click_Count)"]."</td>";
            echo "</tr>";
          }
        }
            ?>
