...
<div class="navbar">
    ...
    <a class="active" href="https://s4515529-infs7901.uqcloud.net/000_q15.php">Current Query</a>
</div>

<div class="row">
  <div class="main"
    <p>Information about Slots Advertiser bid on.</p>
   
    <br>
    <p><b>SQL Query</b></p>
    <p><i>CREATE VIEW advertiser_bid_slot(Bid_Slot) AS(</p>
    <p>SELECT COUNT(SlotID) AS Bid_Slot</p>
    <p>FROM ad</p>
    <p>GROUP BY ad.AdvertiserID)</p>
    <br>
    <p>SELECT $aggregate(Bid_Slot)</p>
    <p>FROM advertiser_bid_slot</i></p>
    <br>
    <?php 
    ...
    <table class="table">
      <thead>
            <tr>
                <th scope="col">Slots Bidded on</th>
            </tr>
        </thead>
      <tbody id="Table">
        <?php
        ...
        // get data
        $query = "SELECT $aggregate(Bid_Slot) 
            FROM advertiser_bid_slot";
        $result = mysqli_query($conn, $query);
        // put all our results into a html table
        while ($rows = mysqli_fetch_array($result)) {
            echo "<tr>";
            echo "<td>".$rows["$aggregate(Bid_Slot)"]."</td>";
            echo "</tr>";
          }
        }
            ?>
